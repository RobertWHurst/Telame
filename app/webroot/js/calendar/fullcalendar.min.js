...
(function(o, K) {
	...
	function Wa(a, b, f, c) {
		function g(m, e, j, r) {
			...
			var s = A.start.getMonth(),I = Ga(new Date),E,B = q(A.visStart);
			if (U) {
				t();
				e = U.find("tr").length;
				if (N < e)U.find("tr:gt(" + (N - 1) + ")").remove(); else if (N > e) {
					m = "";
					for (e = e; e < N; e++) {
				...
				U.find("td").each(function() {
					var W = o(this);
					if (N > 1)B.getMonth() == s ? W.removeClass("fc-other-month") : W.addClass("fc-other-month");
					+B == +I ? W.removeClass("fc-not-today").addClass("fc-today").addClass(ca + "-state-highlight") : W.addClass("fc-not-today").removeClass("fc-today").removeClass(ca + "-state-highlight");
				...
				for (e = 0; e < N; e++) {
					...
					for (E = 0; E < L; E++) {
						m +=
								"<td class='fc-" + za[B.getDay()] + " " + ca + "-state-default fc-day" + (e * L + E) + (E == R ? " fc-leftmost" : "") + (N > 1 && B.getMonth() != s ? " fc-other-month" : "") + (+B == +I ? " fc-today " + ca + "-state-highlight" : " fc-not-today") + "'>" + (r ? "<div class='fc-day-number'>" + B.getDate() + "</div>" : "") + "<div class='fc-day-content'><div style='position:relative'>&nbsp;</div></div></td>";
						C(B, 1);
						O && da(B)
					}
					m += "</tr>"
		...
		function n(m) {
			...
			var e = Fa - x.height(),j;
			if (b.weekMode == "variable")j = e = Math.floor(e / (N == 1 ? 2 : 6)); else {
				j = Math.floor(e / N);
				e = e - j * (N - 1)
			}
			if (Xa === K) {
		...
		function H(m) {
			var e = q(A.visStart),j = C(q(e), L),r = o.map(m, Pa),s,I,E,B,J,W,Ba = [];
			for (s = 0; s < N; s++) {
		...
		function G(m, e) {
			if (!b.disableDragging &&
					e.draggable) {
				var j;
				e.draggable({zIndex:9,delay:50,opacity:A.option("dragOpacity"),revertDuration:b.dragRevertDuration,start:function(r, s) {
		...
		function ma(m, e) {
			for (var j = q(A.visStart),r = C(q(j), L),s = 0; s < N; s++) {
				var I = new Date(Math.max(j, m)),E = new Date(Math.min(r, e));
				if (I < E) {
					var B;
					if (ra) {
						B =
		...
		var ca,ua,O,ra,ka,R,va,Fa,N,L,Aa,x,U,wa = [],P,la = new kb(function(m) {
			return U.find("td:eq(" + (m - Math.max(ua, O) + L) % L + ") div div")
		}),A = o.extend(this, lb, f, {renderGrid:g,renderEvents:y,rerenderEvents:w,clearEvents:t,setHeight:n,setWidth:k,defaultEventEnd:function(m) {
			return q(m.start)
		}});
		A.name = c;
		A.init(a, b);
		...
		var aa = new nb(function(m, e) {
			...
			s[1] = r + j.outerHeight()
		}),Z = new ob(aa),$ = false,ia = pb(A, Z, X, function() {
			return true
		}, ma, ga, Q, ha);
	...
	function jb(a, b, f, c, g, n, k, y, w, t, H) {
		var M = f.options,sa = M.isRTL,G,fa = a.length,D,Q,ha,ma,pa,ga = "",X = {},ca = {},ua = [],O = [];
		for (G = 0; G < fa; G++) {
			D = a[G];
			Q = D.event;
			...
			ga += "<div class='" + ha + Q.className.join(" ") + "' style='position:absolute;z-index:8;left:" + ma + "px'><a" + (Q.url ? " href='" + Ka(Q.url) + "'" : "") + ">" + (!Q.allDay && D.isStart ? "<span class='fc-event-time'>" + Ka(Ha(Q.start, Q.end, f.option("timeFormat"), M)) + "</span>" : "") + "<span class='fc-event-title'>" + Ka(Q.title) + "</span></a>" + ((Q.editable || Q.editable === K && M.editable) && !M.disableResizing &&
					o.fn.resizable ? "<div class='ui-resizable-handle ui-resizable-" + (sa ? "w" : "e") + "'></div>" : "") + "</div>";
			D.left = ma;
			D.outerWidth = pa - ma
		}
		w[0].innerHTML = ga;
		...
		for (G = 0; G < fa; G++) {
			...
			if (k === false)c.remove(); else {
				if (k && k !== true) {
					c.remove();
					c = o(k).css({position:"absolute",left:D.left}).appendTo(w)
				}
				D.element = c;
				if (Q._id === H)t(Q, c, D); else c[0]._fci = G;
	...
	function ub(a, b, f, c) {
		function g(d, i) {
			...
			if (A) {
				...
				ia.find("td").each(function() {
					this.className = this.className.replace(/^fc-\w+(?= )/, "fc-" + za[h.getDay()]);
					+h == +p ? o(this).removeClass("fc-not-today").addClass("fc-today").addClass(na + "-state-highlight") : o(this).addClass("fc-not-today").removeClass("fc-today").removeClass(na + "-state-highlight");
					C(h, Y);
					La && da(h, Y)
				})
			} else {
				var l,u,z = b.slotMinutes % 15 == 0,F = "<div class='fc-agenda-head' style='position:relative;z-index:4'><table style='width:100%'><tr class='fc-first" + (b.allDaySlot ? "" : " fc-last") +
						"'><th class='fc-leftmost " + na + "-state-default'>&nbsp;</th>";
				for (l = 0; l < m; l++) {
				...
				if (b.allDaySlot)F += "<tr class='fc-all-day'><th class='fc-axis fc-leftmost " + na + "-state-default'>" + b.allDayText + "</th><td colspan='" + m + "' class='" + na + "-state-default'><div class='fc-day-content'><div style='position:relative'>&nbsp;</div></div></td><th class='" + na + "-state-default'>&nbsp;</th></tr><tr class='fc-divider fc-last'><th colspan='" +
						(m + 2) + "' class='" + na + "-state-default fc-leftmost'><div/></th></tr>";
				F += "</table></div>";
				A = o(F).appendTo(a);
				w(A.find("td"));
				...
				for (l = 0; l < m; l++) {
					F += "<td class='fc-" + za[h.getDay()] + " " + na +
							"-state-default " + (!l ? "fc-leftmost " : "") + (+h == +p ? na + "-state-highlight fc-today" : "fc-not-today") + "'><div class='fc-day-content'><div>&nbsp;</div></div></td>";
					C(h, Y);
					La && da(h, Y)
				}
				F += "</tr></table></div>";
		...
		function k(d, i) {
			...
			ia.css({top:A.find("tr").height(),height:d});
			i && n()
		}
		...
		function y(d) {
			...
			var i = ia.find("td"),h = aa[0].clientWidth;
			$.width(h);
			j = 0;
		...
		function fa(d) {
			d = Ya(v.sliceSegs(d,
					o.map(d, Pa), v.visStart, v.visEnd));
			var i,h = d.length,p,l,u,z = [];
			for (i = 0; i < h; i++) {
				p = d[i];
		...
		function D(d) {
			var i = ba(q(v.visStart), Da),h = o.map(d, U),p,l,u,z,F,T,S = [];
			for (p = 0; p < m; p++) {
		...
		function ha(d, i) {
			var h,p = d.length,l,u,z,F,T,S,V,ea,ja,xa,ab = "",Ma = {},yb = {};
			for (h = 0; h < p; h++) {
				l = d[h];
				u = l.event;
			...
			for (h = 0; h < p; h++) {
				...
				if (T === false)z.remove(); else {
					if (T && T !== true) {
						z.remove();
						z = o(T).css({position:"absolute",top:l.top,left:l.left}).appendTo(Ba)
					}
					l.element = z;
					if (u._id === i)ga(u, z, l); else z[0]._fci = h;
		...
		function X(d, i, h) {
			if (!b.disableDragging && i.draggable) {
				var p,l = true,u;
				i.draggable({zIndex:9,opacity:v.option("dragOpacity", "month"),revertDuration:b.dragRevertDuration,start:function(F, T) {
		...
		function ca(d, i, h) {
			if (!b.disableDragging && i.draggable) {
				var p,l = false,u,z,F;
				i.draggable({zIndex:9,scroll:false,grid:[r,s],axis:m == 1 ? "y" : false,opacity:v.option("dragOpacity"),revertDuration:b.dragRevertDuration,start:function(V, ea) {
					...
					if (ja && (u || z || l))v.eventDrop(this, d, u, l ? 0 : z, l, V, ea); else {
						S();
						i.css(p);
						T(0);
						o.browser.msie &&
						i.css("filter", "").find("span.fc-event-bg").css("display", "");
						v.showEvents(d, i)
					}
				}});
				function T(V) {
					var ea = ba(q(d.start), V),ja;
		...
		function ua(d, i, h) {
			if (!b.disableResizing && i.resizable) {
				var p,l;
				i.resizable({handles:{s:"div.ui-resizable-s"},grid:s,start:function(u, z) {
					p = l = 0;
					v.hideEvents(d, i);
					...
					if (p)v.eventResize(this, d, 0, b.slotMinutes * p, u, z); else {
						i.css("z-index", 8);
						v.showEvents(d, i)
					}
				}})
		...
		function Aa(d, i) {
			for (var h = q(v.visStart),
					     p = C(q(h), 1),l = 0; l < m; l++) {
				var u = new Date(Math.max(h, d)),z = new Date(Math.min(p, i));
				if (u < z) {
					var F = l * Y + ta;
					F = Ra.rect(0, F, 0, F, Z);
					u = O(h, u);
		...
		var A,aa,Z,$,ia,m,e = 0,j,r,s,I,E,B,J = [],W,Ba,na,vb,La,Ta,Y,ta,Da,Ua,Qa = new kb(function(d) {
			return ia.find("td:eq(" + d + ") div div")
		}),$a = {},v = o.extend(this, lb, f, {renderAgenda:g,renderEvents:M,rerenderEvents:sa,clearEvents:G,setHeight:k,setWidth:y,beforeHide:function() {
			B = aa.scrollTop()
		},afterShow:function() {
			aa.scrollTop(B)
		...
		}),Ea = new ob(Ra),cb = false,Fb = pb(v, Ea, P, la, N, x, ka, R);
		...
		v.dragStart = function(d, i) {
			Ea.start(function(h) {
				x();
				if (h)if (la(h))L(h.row, h.col, h.row, h.col); else {
					h = P(h);
		...
		for (b = a.length - 1; b > 0; b--) {
			...
			for (f = 0; f < g.length; f++) {
				...
				for (c = 0; c < a[b - 1].length; c++) {
					k = a[b - 1][c];
					if (Ab(n, k))k.forward = Math.max(k.forward || 0, (n.forward ||
							0) + 1)
				}
			}
		}
	...
	function xb() {
		var a = 0,b;
		do b = new Date(1970, a++, 1); while (b.getHours());
		return b
	}
	...
	function nb(a) {
		...
		b.cell = function(g, n) {
			var k = f.length,y = c.length,w,t = -1,H = -1;
			...
			return t >= 0 && H >= 0 ? {row:t,col:H} : null
		};
		b.rect = function(g, n, k, y, w) {
			w = w.offset();
			return{top:f[g][0] - w.top,left:c[n][0] - w.left,width:c[y][1] - c[n][0],height:f[k][1] - f[g][0]}
		}
	}
	...
	var ya = o.fullCalendar = {},Oa = ya.views = {},Va = {defaultView:"month",aspectRatio:1.35,header:{left:"title",center:"",right:"today prev,next"},weekends:true,allDayDefault:true,lazyFetching:true,startParam:"start",endParam:"end",
		...
		nextYear:"&nbsp;&lt;&lt;&nbsp;"},buttonIcons:{prev:"circle-triangle-e",next:"circle-triangle-w"}},Db = ya.setDefaults = function(a) {
		o.extend(true, Va, a)
	};
	o.fn.fullCalendar = function(a) {
		if (typeof a == "string") {
			var b = Array.prototype.slice.call(arguments, 1),f;
			this.each(function() {
		...
		c.unshift([]);
		a = o.extend(true,
		{}, Va, a.isRTL || a.isRTL === K && Va.isRTL ? Mb : {}, a);
		var g = a.theme ? "ui" : "fc";
		this.each(function() {
			function n(e) {
				if (e != Aa) {
					...
					if (U[e])(x = U[e]).element.show(); else x = U[e] = ya.views[e](r = wa = o("<div class='fc-view fc-view-" + e + "' style='position:absolute'/>").appendTo(R), a, e);
					if ($) {
			...
			function D(e, j) {
				var r = x.name,s = q(L),I = function(J) {
				...
				if (typeof e == "string") {
					...
					ha();
					o.ajax({url:e,dataType:"json",data:B,cache:a.cacheParam || false,success:E})
					...
					o.each(e.split(" "), function(r) {
						...
						o.each(this.split(","), function(I, E) {
							if (E == "title") {
								...
								var B;
								if (Z[E])B = Z[E]; else if (Oa[E])B = function() {
								...
								if (B) {
									...
									if (I)J = o("<div class='fc-button-" + E + " ui-state-default'><a><span class='ui-icon ui-icon-" + I + "'/></a></div>"); else if (W)J = o("<div class='fc-button-" + E + " " + g + "-state-default'><a><span>" + W + "</span></a></div>");
									if (J) {
										J.click(
										       function() {
			...
			var O = this,ra = o(O).addClass("fc"),ka,R = o("<div class='fc-content " + g + "-widget-content' style='position:relative'/>").prependTo(O),va,Fa = 0,N = 0,L = new Date,Aa,x,U = {},wa;
			a.isRTL && ra.addClass("fc-rtl");
			a.theme && ra.addClass("ui-widget");
			Cb(L, a.year, a.month, a.date);
			var P = [],la,A,aa = 0,Z = {render:function() {
				ga();
				G();
				M();
				k()
			},changeView:n,getView:function() {
				return x
			},getDate:function() {
				return L
			},option:function(e, j) {
				...
				if (typeof e == "object")L = q(e); else Cb(L, e, j, r);
				k()
				...
				if (e) {
					...
					P = o.grep(P, e, true);
					for (r = 0; r < c.length; r++)if (typeof c[r] ==
				...
				if (o.isFunction(e))return o.grep(P, e); else if (e) {
					e += "";
				...
				return P
			},rerenderEvents:H,addEventSource:function(e) {
				c.push(e);
				D(e, H)
			...
			if (a.droppable) {
				var m;
				o(document).bind("dragstart",
				                function(e, j) {
					                var r = e.target,s = o(r);
	...
	Oa.month = function(a, b, f) {
		return new Wa(a, b, {render:function(c, g) {
			...
			C(g,
					(7 - g.getDay() + Math.max(b.firstDay, k)) % 7);
	...
	Oa.basicWeek = function(a, b, f) {
		return new Wa(a, b, {render:function(c, g) {
			...
			this.title = Ha(c, C(q(g), -1), this.option("titleFormat"),
					b);
	...
	var Xa;
	Db({allDaySlot:true,allDayText:"all-day",firstHour:6,slotMinutes:30,defaultEventMinutes:120,axisFormat:"h(:mm)tt",timeFormat:{agenda:"h:mm{ - h:mm}"},
	...
	var lb = {init:function(a, b) {
		...
		var b,f = a.length,c,g = this.eventsByID = {};
		for (b = 0; b < f; b++) {
			c = a[b];
			if (g[c._id])g[c._id].push(c); else g[c._id] = [c]
		}
	},reportEventElement:function(a, b) {
		this.eventElements.push(b);
		var f = this.eventElementsByID;
		if (f[a._id])f[a._id].push(b); else f[a._id] = [b]
		...
		var c = this;
		if (!c.options.disableResizing && b.resizable)b.resizable({handles:c.options.isRTL ? {w:"div.ui-resizable-w"} : {e:"div.ui-resizable-e"},grid:f,minWidth:f / 2,containment:c.element.parent().parent(),start:function(g, n) {
			b.css("z-index", 9);
			c.hideEvents(a, b);
			c.trigger("eventResizeStart",
					this, a, g, n)
		},stop:function(g, n) {
			c.trigger("eventResizeStop", this, a, g, n);
			var k = Math.round((b.width() - n.originalSize.width) / f);
			if (k)c.eventResize(this, a, k, 0, g, n); else {
				b.css("z-index", 8);
				c.showEvents(a, b)
			}
		}})
		...
		for (n = 0; n < k; n++) {
			...
			if (t > f && w < c) {
				...
				g.push({event:y,start:w,end:t,isStart:H,isEnd:M,msLength:t - w})
			}
		}
		return g.sort(Ib)
	}},Bb = 864E5,Jb = 36E5,Hb = 6E4;
	ya.addDays = C;
	ya.cloneDate = q;
	var ib = ya.parseDate = function(a) {
		...
		if (!a)return null;
		var f = new Date(a[1], 0, 1),c = new Date(a[1], 0, 1, 9, 0),g = 0;
		if (a[3]) {
			f.setMonth(a[3] - 1);
			c.setMonth(a[3] - 1)
		}
		...
		c = c || Va;
		var g = a,n = b,k,y = f.length,w,t,H,M = "";
		for (k = 0; k < y; k++) {
			w = f.charAt(k);
