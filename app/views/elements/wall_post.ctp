<?php
if(
	//only show posts that don't belong to the the user, or actions on the news feed
 	(
 		$this->params['action'] == 'news' && 
 		$currentUser['User']['id'] != $post['PostAuthor']['id'] &&
 		$post['WallPost']['type'] != 'action'
 	) ||
 	
 	//show all posts on walls
 	(
 		$this->params['action'] != 'news' 		
 	)
):
